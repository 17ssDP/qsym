...
class FuzzerExecutor(object):
    ...
    def set_asan_cmd(self, asan_bin):
        symbolizer = ""
        for e in [
                "/usr/bin/llvm-symbolizer", "/usr/bin/llvm-symbolizer-3.4",
    ...
    def handle_by_return_code(self, res, fp):
        ...
        if (retcode in [128 + 11, -11, 128 + 6, -6]):
            shutil.copy2(fp, os.path.join(self.my_errors,
                                          os.path.basename(fp)))
    ...
    def report_crash(self, fp):
        ...
        if self.asan_cmd is not None:
            stdout, stderr = utils.run_command(["timeout", "-k", "5", "5"] +
                                               self.asan_cmd, fp)
            info["STDOUT"] = stdout
            info["STDERR"] = stderr
    ...
    def run_file(self, fp):
        ...
        for testcase in q.get_testcases():
            ...
            filename = os.path.join(self.my_queue,
