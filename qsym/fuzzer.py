...
def get_fuzzer_cmd(fuzzer_stats):
...
class FuzzerExecutorState(object):
    def __init__(self):
        self.hang = set()
        self.processed = set()
    ...
    def increase_timeout(self):
        ...
        if self.timeout < MAX_TIMEOUT:
            ...
            # Something bad happened, but wait until Fuzzer resolves it
            logger.debug("Hit the maximum timeout")
            # Back to default timeout not to slow down fuzzing
            self.timeout = DEFAULT_TIMEOUT

        # sleep for a minutes to wait until Fuzzer resolves it
        time.sleep(60)
...
class FuzzerExecutor(object):
    def __init__(self, cmd, output, fuzzer, name, filename=None, mail=None, asan_bin=None):
        self.cmd = cmd
        self.output = output
        self.fuzzer = fuzzer
        self.name = name
        ...
        cmd, fuzzer_path, qemu_mode = self.parse_fuzzer_stats()
    ...
    @property
    def cur_lenconfig(self):
        return os.path.realpath(os.path.join(os.getcwd(), "lenconfig"))
    ...
    @property
    def my_state(self):
        return os.path.join(self.my_queue, ".state")

    @property
    def my_lenconfig(self):
        return os.path.join(self.my_state, "lenconfig")
    ...
    def import_state(self):
        if os.path.exists(self.metadata):
            ...
            self.state = FuzzerExecutorState()

    def sync_files(self):
        files = []
        for name in os.listdir(self.fuzzer_queue):
            path = os.path.join(self.fuzzer_queue, name)
    ...
    def copy_lenconfig(self, fp):
        config_name = os.path.basename(fp).split(",")[0] + ",lenconfig"
    ...
    def run_target(self):
        ...
        q = executor.Executor(self.cmd, self.cur_input,
                              self.tmp_dir, bitmap=self.bitmap, argv=["-l", "1"])
    ...
    def handle_by_return_code(self, res, fp):
        ...
        if (retcode in [128 + 11, -11, 128 + 6, -6]):
            shutil.copy2(fp, os.path.join(
                self.my_errors, os.path.basename(fp)))
    ...
    def run_file(self, fp):
        ...
        for testcase in q.get_testcases():
            ...
            configname = os.path.join(self.my_lenconfig, "id:%06d,lenconfig" % (index))
