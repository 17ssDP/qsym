...
void
analyzeImage(IMG img, void* v) {
  RTN mainMainRtn = RTN_FindByName(img, "main.main");
  if (RTN_Valid(mainMainRtn)) {
    RTN_Open(mainMainRtn);
    INS ins = RTN_InsHeadOnly(mainMainRtn);
    main_addr = INS_Address(ins); 
    LOG_DEBUG("The address of the first intruction of main.main is " + hexstr(main_addr) + "\n");
    RTN_Close(mainMainRtn);
    analyzeMain(mainMainRtn);
  }
  RTN mainRtn = RTN_FindByName(img, "main");
  if (RTN_Valid(mainRtn)) {
    RTN_Open(mainRtn);
    INS ins = RTN_InsHeadOnly(mainRtn);
    main_addr = INS_Address(ins);
    LOG_DEBUG("The address of the first intruction of main is " +
              hexstr(main_addr) + "\n");
    RTN_Close(mainRtn);
    analyzeMain(mainRtn);
  }
...
analyzeInstruction(INS ins) {
  ...
  LOG_DEBUG("Thread " + decstr(tid) + " Meet instruction: " +
            INS_Disassemble(ins) + " at " + hexstr(ins_addr) + "\n");

  // if (!enter_main) {
  //   LOG_DEBUG("Ignore this instruction\n");
  //   return;
  // }
